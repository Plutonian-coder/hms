...
import { CheckCircle2, XCircle, Zap, Filter, Search } from "lucide-react";
...
export default function ApplicationsPage() {
  ...
  const fetchApplications = async () => {
    try {
      const res: any = await api.get("/admin/applications", token!);
  ...
  const verifyPayment = async (appId: string) => {
    try {
      await api.patch(`/admin/applications/${appId}/verify`, { verified: true }, token!);
